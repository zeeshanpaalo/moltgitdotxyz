...
// settings
var (
	// AppVer is the version of the current build of MoltGit. It is set in main.go from main.Version.
	AppVer string
	// AppBuiltWith represents a human-readable version go runtime build version and build tags. (See main.go formatBuiltWith().)
	AppBuiltWith string
	// AppStartTime store time MoltGit has started
...
func PrepareAppDataPath() error {
	...
	// For example, if someDir=/mnt/vol1/moltgit-home/data, if the mount point /mnt/vol1 is not mounted when MoltGit runs,
	// then MoltGit will make new empty directories in /mnt/vol1, all are stored in the root filesystem.
...
func MustInstalled() {
	if !InstallLock {
		log.Fatal(`Unable to load config file for an installed MoltGit instance, you should either use "--config" to set your config file (app.ini), or run "moltgit web" command to install MoltGit.`)
	}
}
...
func loadRunModeFrom(rootCfg ConfigProvider) {
	...
	// The following is a purposefully undocumented option. Please do not run MoltGit as root. It will only cause future headaches.
	...
	if os.Getuid() == 0 {
		if !unsafeAllowRunAsRoot {
			...
			log.Fatal("MoltGit is not supposed to be run as root. Sorry. If you need to use privileged TCP ports please instead use setcap and the `cap_net_bind_service` permission")
		}
		log.Critical("You are running MoltGit using the root user, and have purposely chosen to skip built-in protections around this. You have been warned against this.")
	}
}
