...
func TestDecodeEnvironmentKey(t *testing.T) {
	...
	// with "__FILE" suffix, it doesn't support to write "[sec].FILE" to config (no such key FILE is used in MoltGit)
	// but it could be fixed in the future by adding a new suffix like "__VALUE" (no such key VALUE is used in MoltGit either)
